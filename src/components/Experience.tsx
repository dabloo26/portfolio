            ...
            {experience.map((job, i) => (
              ...
              >
                <span
                  ...
                  aria-hidden
                />
                <div className="min-w-0 max-w-full overflow-hidden rounded-xl border border-white/[0.1] bg-[#070a12]/55 p-3 text-zinc-100 shadow-[0_8px_32px_rgba(0,0,0,0.3)] backdrop-blur-md sm:p-4 md:p-5">
                  <div className="flex min-w-0 flex-wrap items-baseline justify-between gap-x-2 gap-y-1">
                    <p className="font-mono text-[10px] uppercase tracking-wide text-accent-acid sm:text-xs">
                      {job.period}
                    </p>
                    {job.timelineNote ? (
                      <p className="hidden max-w-[min(100%,14rem)] font-mono text-[9px] leading-tight text-zinc-400 md:block md:max-w-none md:text-[10px]">
                        {job.timelineNote}
                      </p>
                    ) : null}
                  </div>
                  <h3 className="mt-1.5 text-[15px] font-semibold leading-snug text-neutral-50 sm:mt-2 sm:text-base md:text-lg">
                    {experienceTitle(job)}
                  </h3>
                  <p className="mt-0.5 text-[11px] font-medium uppercase tracking-[0.1em] text-zinc-300 sm:text-xs">
                    {job.company}
                  </p>
                  <ul className="mt-2.5 space-y-1.5 border-t border-white/[0.06] pt-2.5 text-[12px] leading-snug sm:mt-3 sm:space-y-2 sm:pt-3 sm:text-[13px] sm:leading-relaxed md:text-sm">
                    {experienceBulletsForViewport(job, isNarrow).map((b, j) => (
                      <li key={j} className="flex min-w-0 gap-2">
                        ...
                        <span className="min-w-0 break-words text-pretty text-zinc-200">{b}</span>
                      </li>
                    ))}
                  </ul>
                </div>
